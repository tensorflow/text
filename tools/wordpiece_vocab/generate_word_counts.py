...
def word_count(input_path, output_path, temp_dir, raw_metadata,
  ...
  def pipeline(root):
    """Creates a pipeline to generate sorted wordcount over a corpus."""
    ...
    with tft_beam.Context(temp_dir=temp_dir):
      ...
      (transformed_data, _), _ = (
          ...
          | 'FilterLangAndExtractToken' >> tft_beam.AnalyzeAndTransformDataset(
              utils.count_tokenizing_fn(FLAGS.text_key,
...
def main(_):
  ...
  pipeline = word_count(FLAGS.input_path, FLAGS.output_path, temp_dir,
                        raw_metadata)
