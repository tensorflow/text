...
template <tflite::shim::Runtime Rt>
  ...
  for (int i = 0; i < input_values.Dim(0); ++i) {
    const int orig_num_bytes = output_values.size();
    for (int j = in_splits(i); j < in_splits(i+1); ++j) {
      output_values.push_back(std::string(input_values(i), starts(j),
                                          ends(j) - starts(j)));
    }
    const int delta_num_bytes = output_values.size() - orig_num_bytes;
    out_splits.push_back(delta_num_bytes + out_splits.back());
  }
