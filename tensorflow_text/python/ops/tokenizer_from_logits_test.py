...
from tensorflow_text.python.ops.tokenizer_from_logits import TokenizerFromLogits  # pylint: disable=line-too-long
...
class TokenizerFromLogitsTest(test.TestCase):

  def setUp(self):
    super(TokenizerFromLogitsTest, self).setUp()
    self.tokenizer = TokenizerFromLogits()
  ...
  def testVectorSingleValueTokenAcrossSpace(self):
    ...
    (tokens, starts, limits) = (
        self.tokenizer.tokenize_with_offsets(
            test_strings, test_logits, force_split_at_break_character=False))
    self.assertAllEqual(tokens, expected_tokens)
    self.assertAllEqual(starts, expected_start_offsets)
    self.assertAllEqual(limits, expected_limit_offsets)
