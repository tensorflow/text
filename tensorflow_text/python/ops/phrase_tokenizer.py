...
class PhraseTokenizer(Tokenizer, Detokenizer):
  ...
  are as follows (as of 11/2021):
    ...
    * `unknown_token` cannot be None or empty. That means if a word is too long
      or cannot be tokenized, PhraseTokenizer always returns
    ...
    * By default the input is assumed to be general text (i.e., sentences), and
      PhraseTokenizer first splits it on whitespaces and
      then applies the Wordpiece tokenization.
      TODO(yunzhu): support split on punctuations as well.
  """
    ...
    if model_buffer is None:
      model_buffer = (
          pywrap_phrase_tokenizer_model_builder.build_phrase_model(
              vocab, unknown_token, support_detokenization, width))
  ...
  def tokenize(self, input):  # pylint: disable=redefined-builtin
    ...
    >>> tokenizer = PhraseTokenizer(vocab, token_out_type=tf.string, width=3)
  ...
  def detokenize(self, input):  # pylint: disable=redefined-builtin
    ...
    """
    name = None
    with ops.name_scope(name, 'PhraseDetokenize', [input, self._model]):
      # Check that the types are expected and the ragged rank is appropriate.
