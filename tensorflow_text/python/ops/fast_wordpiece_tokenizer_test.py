...
class FastWordpieceOpOriginalTest(test_util.TensorFlowTestCase,
  ...
  def notestWordPieceOpAndVerifyOffsets(self,
                                        tokens,
                                        expected_subwords,
                                        vocab,
                                        expected_start=None,
  ...
  def notestWordPieceOpWithMultipleRaggedRank(self,
                                              tokens,
                                              expected_subwords,
                                              vocab,
                                              expected_start=None,
  ...
  def notestWordPieceOpWithIdReturned(self):
  ...
  def notestWordPieceOpWithInt32IdReturned(self):
    """Let the table determine how to do a lookup on unknown tokens."""
    tokens = ragged_factory_ops.constant(
        [[b"don't", b"tread", b"cantfindme", b"treadcantfindme"]])
    tokenizer = FastWordpieceTokenizer(
...
class FastWordpieceOpAdditionalTest(test_base.DatasetTestBase,
  ...
  def notestTokenizerBuiltFromConfig(self, text_inputs, expected_outputs):
    tokenizer = FastWordpieceTokenizer(
        vocab=_TEST_VOCAB,
  ...
  def notestTokenizerBuiltFromModel(self, text_inputs, expected_outputs):
    model_buffer = _LoadTestModelBuffer()
    tokenizer = FastWordpieceTokenizer(model_buffer=model_buffer)

    self.assertAllEqual(tokenizer.tokenize(text_inputs), expected_outputs)

  def notestTokenizerBuiltInsideTfFunctionFromConfig(self, text_inputs,
                                                     expected_outputs):
  ...
  def notestTokenizerBuiltInsideTfFunctionFromModel(self, text_inputs,
                                                    expected_outputs):
  ...
  def notestTokenizerBuiltOutsideTfFunctionFromConfig(self, text_inputs,
                                                      expected_outputs):
    tokenizer = FastWordpieceTokenizer(
        vocab=_TEST_VOCAB,
        max_bytes_per_word=_TEST_MAX_BYTES_PER_WORD,
  ...
  def notestTokenizerBuiltOutsideTfFunctionFromModel(self, text_inputs,
                                                     expected_outputs):
    model_buffer = _LoadTestModelBuffer()
    tokenizer = FastWordpieceTokenizer(model_buffer=model_buffer)
...
class EndToEndFastWordpieceOpTest(test_base.DatasetTestBase,
  ...
  def NO_testTokenizerBuiltFromConfig(self, text_inputs, expected_outputs):
    tokenizer = FastWordpieceTokenizer(
        vocab=_TEST_VOCAB,
        max_bytes_per_word=_TEST_MAX_BYTES_PER_WORD,
        suffix_indicator=_TEST_SUFFIX_INDICATOR,
...
class FastWordpieceDetokenizeOpTest(test_base.DatasetTestBase,
  ...
  def NO_testTokenizerBuiltFromConfig(self, id_inputs, expected_outputs):
    tokenizer = FastWordpieceTokenizer(
        vocab=_TEST_VOCAB,
        max_bytes_per_word=_TEST_MAX_BYTES_PER_WORD,
        suffix_indicator=_TEST_SUFFIX_INDICATOR,
...
class FastWordpieceInKerasModelTest(test_util.TensorFlowTestCase,
  ...
  def notestTfliteWordpieceTokenizer(self, end_to_end, text_inputs):
