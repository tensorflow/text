...
gen_tokenizer_from_logits = load_library.load_op_library(resource_loader.get_path_to_datafile('_tokenizer_from_logits.so'))
from tensorflow_text.python.ops.tokenization import TokenizerWithOffsets

_tf_text_tokenizer_from_logits_op_create_counter = monitoring.Counter(
    '/nlx/api/python/tokenizer_from_logits_create_counter',
    'Counter for number of TokenizerFromLogits instances created in Python.')


class TokenizerFromLogits(TokenizerWithOffsets):
  """Tokenizes a tensor of UTF-8 string into words according to logits."""

  def __init__(self):
    """Initializes a new instance.
    """
    super(TokenizerFromLogits, self).__init__()
    _tf_text_tokenizer_from_logits_op_create_counter.get_cell().increase_by(1)

  def tokenize(self, strings, logits, force_split_at_break_character=True):
  ...
  def tokenize_with_offsets(self, strings, logits,
    ...
    Args:
      ...
      force_split_at_break_character: bool indicates whether to force start a
        new word after seeing an ICU-defined whitespace character.  For more
        info, please see the comments for the tokenize() method.
    ...
    name = None
    with ops.name_scope(
        name, 'TokenizerFromLogits',
        [strings, logits, force_split_at_break_character]):
      # Tokenize the strings into tokens.
      token_values, token_row_splits, start_values, limit_values = (
          gen_tokenizer_from_logits.tokenizer_from_logits(
              strings=strings,
              logits=logits,
