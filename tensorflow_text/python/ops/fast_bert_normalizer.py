...
class FastBertNormalizer(module.Module):
  ...
  def normalize(self, input):  # pylint: disable=redefined-builtin
    ...
    """
    normalized_texts, _ = self._normalize_with_offsets_map_helper(
        input, get_offsets_map=False)
  ...
  def normalize_with_offsets_map(self, input):  # pylint: disable=redefined-builtin
    ...
    Returns:
      ...
      offsets_map: is a `RaggedTensor` of the byte offsets from the output
        to the input.
    """
    return self._normalize_with_offsets_map_helper(input, get_offsets_map=True)

  def _normalize_with_offsets_map_helper(self, input, get_offsets_map):  # pylint: disable=redefined-builtin
    r"""The actual function of normalization.

    Args:
      input: An N-dimensional `Tensor` or `RaggedTensor` of UTF-8 strings.
      get_offsets_map: bool. Whether to compute offsets_map or not.

    Returns:
      A tuple `(texts, offsets_map)` where:
        texts: is a `Tensor` or `RaggedTensor`.
        offsets_map: If `get_offsets_map`=True, it is a `RaggedTensor` of the
    ...
    with ops.name_scope(name, 'FastBertNormalize', [input, self._model]):
      ...
      if rank == 0:
        normalized_texts, offsets_map = self._normalize_with_offsets_map_helper(
            array_ops.stack([input]), get_offsets_map)
        return (normalized_texts.values,
                offsets_map.values if get_offsets_map else None)
        ...
        normalized_texts, offsets_map = self._normalize_with_offsets_map_helper(
            input.flat_values, get_offsets_map)
        if get_offsets_map:
          offsets_map = offsets_map.with_row_splits_dtype(
              input.row_splits.dtype)
        return (input.with_flat_values(normalized_texts),
                input.with_flat_values(offsets_map)
                if get_offsets_map else None)
      ...
      normalized_texts, offsets_map, row_splits = (
          gen_fast_bert_normalizer.fast_bert_normalize(
              ...
              get_offset_mappings=get_offsets_map))
