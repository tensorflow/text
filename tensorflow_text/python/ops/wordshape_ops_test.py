...
class Utf8CharsOpTest(test.TestCase):
  ...
  def testDashShape(self):
    test_string = [u"a-b", u"a\u2010b".encode("utf-8"),
                   ...
                   u"abc".encode("utf-8")]
    shapes = text.wordshape(test_string, text.WordShape.HAS_PUNCTUATION_DASH)
  ...
  def testNoPunct(self):
    ...
    shapes = text.wordshape(test_string, text.WordShape.HAS_NO_PUNCT_OR_SYMBOL)
    self.assertAllEqual(shapes, [True, False])
  ...
  def testEndsWithEmoticon(self):
    ...
    shapes = text.wordshape(test_string, text.WordShape.ENDS_WITH_EMOTICON)
  ...
  def testAcronym(self):
    ...
    shapes = text.wordshape(test_string, text.WordShape.IS_ACRONYM_WITH_PERIODS)
  ...
  def testMixedCaseLetters(self):
    ...
    shapes = text.wordshape(test_string, text.WordShape.IS_MIXED_CASE_LETTERS)
  ...
  def testOpenQuote(self):
    ...
    shapes = text.wordshape(test_string, text.WordShape.BEGINS_WITH_OPEN_QUOTE)
    self.assertAllEqual(shapes, [False, False, True, True, False, True])

  def testCloseQuote(self):
    test_string = [u"''", u"ABc\"", u"\u300f".encode("utf-8"),
                   u"\u2018".encode("utf-8"), u"aBcd", u"``"]
    shapes = text.wordshape(test_string, text.WordShape.ENDS_WITH_CLOSE_QUOTE)
  ...
  def testCurrencySymbol(self):
    test_string = [u"''", u"ABc$", u"$\uff07".encode("utf-8")]
    shapes = text.wordshape(test_string, text.WordShape.HAS_CURRENCY_SYMBOL)
    self.assertAllEqual(shapes, [False, True, False])
  ...
  def testMultipleShapes(self):
    ...
    shapes = text.wordshape(
        test_string, [text.WordShape.IS_UPPERCASE, text.WordShape.IS_LOWERCASE])
    self.assertAllEqual(shapes, [[False, True], [False, False], [True, False]])
