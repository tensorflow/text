...
class FastWordpieceTokenizer(TokenizerWithOffsets, Detokenizer):
  ...
  def __init__(self,
    ...
    if model_buffer is None:
      model_buffer = pywrap_fast_wordpiece_builder.build_fast_wordpiece_model(
          vocab, max_bytes_per_word, suffix_indicator, unknown_token,
  ...
  def detokenize(self, input):  # pylint: disable=redefined-builtin
    ...
    >>> ids = [[0, 1, 2], [3]]  # Dummy ids of ["they're the greatest", "ok"]
    >>> tokenizer = FastWordpieceTokenizer(vocab)
    >>> tokenizer.detokenize(ids)
    ["they're the greatest", "ok"]
